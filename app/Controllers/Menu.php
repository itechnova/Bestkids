...
class Menu extends BaseController
{
    ...
    {
        $this->titlePage = $this->viewContent()->orders->titlePage;
        $this->description = $this->viewContent()->orders->description;
        $this->setContent($this->viewContent()->orders->title, $this->viewContent()->orders->content);
        $this->addBreadcrumb($this->titlePage);
        $this->withLayout = 'orders';
        // Verificar si el usuario ya está autenticado
        if (!(session()->get('isLoggedIn'))) {
            return redirect()->to('/login');
        }

        return $this->View('settings/menu/order');
    }
    ...
    {
        if($this->withLayout !== 'orders'){
            return '';
        }
        ob_start(); ?>
        <!-- Nestable -->
        <link rel="stylesheet" href="<?=base_url('public/vendors/nestable/nestable.css');?>" type="text/css">
    ...
    protected function script(): string
    {
        $Model = $this->getModel();
        ob_start(); ?>
        <?php if($this->withLayout === 'orders'){ ?>
            <script src="<?=base_url('public/vendors/nestable/jquery.nestable.js');?>"></script>
            <script>
                $(function () {
                    ...
                    $('#nestable-menu').on('click', function (e) {
                        ...
                        if(action === 'saved-all'){
                            $.ajax({
                                url: '<?=site_url($this->slug."/orders/saved");?>',   // URL a la que enviar la solicitud
                                method: 'POST',      // Método HTTP (POST, GET, etc.)
                                data: {
                                    idsetting: '<?=SETTING('menu_order', 'menu')->idsetting;?>',
                                    groups: 'menu',
                                    name: 'menu_order',
                                    value: $('textarea#nestable-output').val()
